...
class ServicioControllerIntegrationTest {
    ...
    @Test
    @DisplayName("POST servicio con precio negativo → 422 Unprocessable Entity")
    void crearServicio_precioNegativo_retorna422() throws Exception {
        Servicio invalido = new Servicio("Corte", "desc", -100, 60);

        mockMvc.perform(post("/api/servicios")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(invalido)))
                .andExpect(status().isUnprocessableEntity())
                .andExpect(jsonPath("$.mensaje").value(containsString("mayor a cero")));
    }
